...
def remove_stats_nans(stats: List[Stat]) -> List[Stat]:
    ...
    for stat in stats:
        if math.isnan(stat.sum):
            hlog(
                f"WARNING: Removing stat {stat.name.name} because its value"
                " is NaN"
            )
            continue
        result.append(stat)
...
def downsample_eval_instances(
    instances: List[Instance], max_eval_instances: int
) -> List[Instance]:
    """
    ...
    hlog(
        f"{len(instances)} instances, {len(all_train_instances)} train"
        " instances,"
        f" {len(selected_eval_instances)}/{len(all_eval_instances)} eval"
        " instances"
    )
...
class Runner:
    ...
    def run_one(self, run_spec: RunSpec):
        ...
        if self.skip_completed_runs and self._is_run_completed(run_path):
            hlog(
                f"Skipping run {run_spec.name} because run is completed and all"
                " output files exist."
            )
            return
        ...
        if self.skip_instances:
            ...
            if self.cache_instances and os.path.exists(
                ...
                scenario_output_path = get_scenario_cache_path(
                    self.output_path, scenario.name
                )
