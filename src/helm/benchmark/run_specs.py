...
def get_machine_translation_metric_specs() -> List[MetricSpec]:
    return [
        MetricSpec(
            class_name="helm.benchmark.metrics.machine_translation_metrics.MachineTranslationMetric",
            args={},
        )
    ] + get_basic_metric_specs([])
...
def get_commonsense_spec(dataset: str, method: str) -> RunSpec:
    scenario_spec = ScenarioSpec(
        class_name=(
            "helm.benchmark.scenarios.commonsense_scenario.CommonSenseScenario"
        ),
...
def construct_run_specs(spec: ObjectSpec) -> List[RunSpec]:
    ...
    def alter_run_spec(run_spec: RunSpec) -> RunSpec:
        ...
        if (
            OPENAI_CHATGPT_MODEL_TAG in model.tags
            and run_spec.adapter_spec.max_tokens
        ):
